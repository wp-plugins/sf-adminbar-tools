...
function sf_abt_enable_coworking() {
	...
	if ( !($userdata = get_userdata($uid)) )
		exit();

	// Update the user meta
	$cowork	= (int) get_user_meta($uid, 'sf-abt-coworking', true);
...
function sf_abt_update_open_files() {
	...
	if ( isset($_POST['file']) && $_POST['file'] ) {
		...
		if ( !isset($open_files[$file]) || !$open_files[$file] ) {		// File is not open yet, the user can assign it to himself. We return the avatar
			$open_files[$file] = $uid;
			update_option('sf-abt-open-files', $open_files);
			$msg = array( 'msg' => 'you-got-it-my-young-padawan', 'avatar' => get_abt_avatar($uid, $userdata->display_name), 'files' => $open_files );
			...
			$ouid = (int) $open_files[$file];
			if ( !($otheruserdata = get_userdata($ouid)) ) {
				echo '{"msg":"error"}';
				exit();
			}
			$msg = array( 'msg' => 'you-shall-not-pass', 'avatar' => get_abt_avatar($ouid, $otheruserdata->display_name), 'files' => $open_files );
		}
